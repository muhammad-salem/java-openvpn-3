...
public class OpenVPNClientThread extends OpenVPNClient implements Runnable {
	private EventReceiver parent;
	private TunBuilder tun_builder;
	private Thread thread;
	private Status m_connect_status;
	...
	public interface EventReceiver {
		...
		void external_pki_cert_request(ExternalPKICertRequest req);

		// Callback to sign data
		void external_pki_sign_request(ExternalPKISignRequest req);
	}
	...
	private void call_done(Status status) {
		EventReceiver p = finalize_thread(status);
		if (p != null)
			p.done(m_connect_status);
	}

	private synchronized EventReceiver finalize_thread(Status connect_status) {
	...
	public void run() {
		...
		Status status = super.connect();
		call_done(status);
	}

	// OpenVPNClient (C++ class) overrides
	...
	@Override
	public boolean tun_builder_add_address(String address, int prefix_length, String gateway, boolean ipv6,
			boolean net30) {
		TunBuilder tb = tun_builder;
		if (tb != null)
